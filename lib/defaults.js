...
function defaultOnExceptionMap(onExceptionMap) {
  if (onExceptionMap !== undefined) {
    if (!(typeof onExceptionMap === 'function')) {
      throw TypeError('function required for onExceptionMap');
    }
  }

  function noopOnExceptionMap(error) {
    return error;
  }

  return onExceptionMap || noopOnExceptionMap;
}
...
function defaults(options) {
  return {
    ...
    onExceptionMap: defaultOnExceptionMap(options && options.onExceptionMap),
  };
}
